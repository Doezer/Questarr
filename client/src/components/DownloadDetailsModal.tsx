    ...
    <div className="flex-1 flex flex-col overflow-hidden min-h-0 px-4 pb-4">
      ...
      {details && (
        <Tabs defaultValue="info" className="flex-1 flex flex-col overflow-hidden min-h-0">
          {/*
            Older API responses may not provide filesSupport yet; treat them as supported
            for backward-compatible rendering.
          */}
          {(() => {
            const filesSupport = details.filesSupport ?? "supported";
            const filesCountLabel = filesSupport === "supported" ? details.files.length : "N/A";
                  ...
                  <ScrollArea className="h-full pr-4">
                    {filesSupport === "unsupported" ? (
                      <div
                        ...
                        data-testid="files-unsupported"
                      >
                        {details.filesSupportReason ||
