...
Extra directories scanned (read-only discovery) for games already on disk outside the configured library root (e.g. an older library, a secondary drive). Separate from the library root used by the download-import pipeline — matched games get a `libraryPath` outside that root, and Questarr never moves, renames, or deletes files there.

| Method | Path                                 | Auth Required                                                                       | Request Body                                                                                                 | Response                                                                                |
| ------ | ------------------------------------ | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------- |
| GET    | `/api/root-folders`                  | JWT                                                                                 | —                                                                                                            | `RootFolder[]`                                                                          |
| POST   | `/api/root-folders`                  | JWT + `sensitiveEndpointLimiter`, `sanitizeRootFolderData`, `validateRequest`       | `{ path (1–1000, no null bytes), name? (≤200), enabled? }` via `insertRootFolderSchema`; path probed on disk | 201 `RootFolder`; 400 if path inaccessible; 409 if path already registered              |
| PATCH  | `/api/root-folders/:id`              | JWT + `sensitiveEndpointLimiter`, `sanitizeRootFolderUpdateData`, `validateRequest` | Partial: `path?, name?, enabled?`                                                                            | Updated `RootFolder`; 404 if not found; 409 if new path already registered              |
| DELETE | `/api/root-folders/:id`              | JWT + `sensitiveEndpointLimiter`                                                    | —                                                                                                            | 204; 404 if not found                                                                   |
| POST   | `/api/root-folders/:id/health-check` | JWT + `sensitiveEndpointLimiter`                                                    | —                                                                                                            | Force-refreshed accessibility + disk stats for the folder; 404 if not found             |
| POST   | `/api/library/scan`                  | JWT + `sensitiveEndpointLimiter`                                                    | `{ rootFolderId? }` — omit to scan all enabled root folders                                                  | 202 `{ accepted: true, rootFolderId }`; scan runs asynchronously                        |
...
| POST   | `/api/library/scan/unmatched/match`  | JWT + `sensitiveEndpointLimiter`                                                    | `{ rootFolderId, folderName, igdbId }`                                                                       | `{ gameId, filesAdded }`; 400 if fields missing; 500 with message on resolution failure |
