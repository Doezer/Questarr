...
const MIGRATION_CONFIG: Record<string, TableConfig> = {
  ...
  user_settings: {
    targetColumns: [
      "id", "user_id", "auto_search_enabled", "auto_download_enabled",
      "notify_multiple_downloads", "notify_updates", "search_interval_hours",
      "igdb_rate_limit_per_second", "download_rules", "last_auto_search", "updated_at"
    ],
  ...
  games: {
    targetColumns: [
      "id", "user_id", "igdb_id", "title", "summary", "cover_url",
      "release_date", "rating", "platforms", "genres", "publishers",
      "developers", "screenshots", "status", "original_release_date",
      "release_status", "hidden", "added_at", "completed_at"
    ],
    requiredSourceColumns: ["title"]
  },
  indexers: {
    targetColumns: [
      "id", "name", "url", "api_key", "protocol", "enabled", "priority",
      "categories", "rss_enabled", "auto_search_enabled", "created_at", "updated_at"
    ],
  ...
  downloaders: {
    targetColumns: [
      "id", "name", "type", "url", "port", "use_ssl", "url_path",
      "username", "password", "enabled", "priority", "download_path",
      "category", "label", "add_stopped", "remove_completed",
      "post_import_category", "settings", "created_at", "updated_at"
    ],
  ...
  game_downloads: {
    ...
    targetColumns: [
      "id", "game_id", "downloader_id", "download_type", "download_hash",
      "download_title", "status", "added_at", "completed_at"
    ],
    ...
    for (const targetTableName of tables) {
      ...
      try {
        ...
        try {
            ...
            if (err.code === "42P01" && config.sourceTableAliases) {
                ...
                for (const alias of config.sourceTableAliases) {
                    try {
                        console.log(`   Target table ${sourceTableName} not found in source, trying alias: ${alias}`);
        ...
        if (rows.length === 0) {
            ...
            if (sourceTableName !== targetTableName && !config.sourceTableAliases?.includes(sourceTableName)) {
                 console.log(`   Source table ${targetTableName} (and aliases) not found in Postgres, skipping.`);
            } else {
            ...
            for (let row of rowsToInsert) {
                ...
                const values = targetCols.map(targetCol => {
                    ...
                    if (val === undefined && config.columnMapping) {
                         ...
                         const sourceKey = Object.keys(config.columnMapping).find(k => config.columnMapping![k] === targetCol);
        ...
        stats[targetTableName] = { source: sourceRowCount, migrated: result.migrated, skipped: result.skipped };
        
        console.log(`   ✅ Migrated: ${result.migrated}, Skipped (Duplicate): ${result.skipped}`);

        // 5. Post-Migration Validation
        const targetCount = sqlite.prepare(`SELECT COUNT(*) as count FROM "${targetTableName}"`).get() as { count: number };
        
        if (targetCount.count < result.migrated) {
            console.error(`   ⚠️ Integrity Check Failed: Target has ${targetCount.count} rows, expected at least ${result.migrated}`);
        }
