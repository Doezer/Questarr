    ...
    test('should perform initial setup and save auth state', async ({ page, request }) => {
        ...
        if (!hasUsers) {
            ...
            const [response] = await Promise.all([
                page.waitForResponse(resp => resp.url().includes('/api/auth/setup') && resp.status() === 200),
                submitBtn.click()
            ]);
            ...
            try {
                ...
                console.log('Setup API 200 OK, but redirect timeout.');
                const errorText = await page.locator('.text-destructive').textContent().catch(() => 'No UI error');
                console.log('UI Error:', errorText);

                // Workaround: Manually go to / if stuck
                if (page.url().endsWith('/setup')) {
                    console.log('Manually navigating to / after success...');
                    await page.goto('/');
                }
            }
            ...
            console.log('Setup already completed, unexpected for clean run.');
        }
    });
});
