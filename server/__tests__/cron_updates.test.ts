...
describe('checkGameUpdates', () => {
  ...
  it('should batch updates for multiple games', async () => {
    ...
    const mockGames: Partial<Game>[] = [
      {
        ...
        releaseStatus: 'upcoming', // Should change to released
      ...
      },
      {
        ...
        releaseStatus: 'released', // Should change to upcoming
      ...
      },
      {
        ...
        originalReleaseDate: '2024-01-01',
      }
    ];
    ...
    const mockIgdbGames = [
      ...
      {
        id: 1003,
        first_release_date: Math.floor(new Date('2024-01-01').getTime() / 1000),
      }
    ];
    vi.mocked(igdbClient.getGamesByIds).mockResolvedValue(mockIgdbGames as any);

    vi.mocked(storage.addNotification).mockResolvedValue({ id: 'notif-1' } as any);
