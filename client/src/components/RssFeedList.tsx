...
export default function RssFeedList() {
  const { data: items, isLoading, refetch } = useQuery<RssFeedItem[]>({
          ...
          <Card key={item.id} className="overflow-hidden flex flex-col h-full hover:shadow-md transition-shadow">
             {item.coverUrl && (
                <div className="aspect-video w-full overflow-hidden bg-muted relative">
                   <img src={item.coverUrl} alt={item.igdbGameName || item.title} className="w-full h-full object-cover" />
                   {item.igdbGameName && (
            ...
            <CardHeader className="p-4 pb-2 flex-grow">
              <div className="flex justify-between items-start gap-2 mb-2">
                 <Badge variant="secondary" className="text-xs">{item.sourceName}</Badge>
            ...
            <CardContent className="p-4 pt-0 mt-auto">
               <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline flex items-center gap-1 mt-2">
