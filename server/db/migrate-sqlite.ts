...
/**
 * SQLite migration runner and schema-repair logic.
 *
 * Moved verbatim from server/migrate.ts when Postgres support was added. This
 * is deliberately NOT shared with Postgres: it walks the drizzle-kit journal by
 * hand, splits statements on the "--> statement-breakpoint" marker, swallows
 * "already exists" errors, and repairs drifted installs with PRAGMA
 * table_info() and rowid-based dedup. All of that exists to rescue databases
 * that predate a migration file, which cannot happen on a greenfield Postgres
 * install. See server/db/migrate-pg.ts.
 */
 ...
 */
export async function repairSqliteSchemaForV1_3_0(): Promise<void> {
