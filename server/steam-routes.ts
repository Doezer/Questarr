...
const STEAM_API_KEY = process.env.STEAM_API_KEY || "YOUR_STEAM_API_KEY"; // NEEDS CONFIG OR ENV
...
router.get('/api/auth/steam', authenticateToken, (req, res, next) => {
    ...
    // We can set a cookie 'steam_auth_user' to the userId.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const user = (req as any).user;
    res.cookie('steam_auth_user', user.id, { httpOnly: true, maxAge: 5 * 60 * 1000 }); // 5 mins
    ...
    const strategy = passport._strategies['steam'] as any;
    if (strategy) {
...
router.get('/api/auth/steam/return', (req, res, next) => {
    ...
    passport.authenticate('steam', { session: false, failureRedirect: '/settings?error=steam_auth_failed' }, async (err, user) => {
        if (err || !user) {
        ...
        // This req.cookies requires cookie-parser? Express default doesn't parse cookies?
        // Check index.ts
        // index.ts: app.use(express.json()) ... no cookie-parser.
        // We can parse 'req.headers.cookie' manually or install cookie-parser.
        ...
        const userId = cookies?.['steam_auth_user'];
        ...
        try {
             await storage.updateUserSteamId(userId, steamId);
             res.clearCookie('steam_auth_user');
