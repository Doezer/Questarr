...
- The `overrides` field forces a specific version of a transitive dependency when a direct dependency's own declared range still permits a vulnerable release:
  ...
  - `proxy-addr: ^2.0.8` patches [CVE-2026-90711](https://vuldb.com/cve/CVE-2026-90711) (critical: accepts undersized IPv4-mapped IPv6 trust subnets, letting unauthenticated clients spoof `X-Forwarded-For` and bypass IP-based access controls, rate limiting, and audit logging), affecting `>=1.1.0 <=2.0.7`. Needed because `express` pins `proxy-addr: ~2.0.7`, which had not yet bumped its declared range to include the patched `2.0.8`.
