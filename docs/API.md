...
| PATCH  | `/api/root-folders/:id`              | JWT + `sensitiveEndpointLimiter`, `sanitizeRootFolderUpdateData`, `validateRequest` | Partial: `path?, name?, enabled?, allowDelete?`                                                                                                               | Updated `RootFolder`; 404 if not found; 409 if new path already registered              |
