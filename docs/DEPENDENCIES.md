...
- The `overrides` field forces a specific version of a transitive dependency when a direct dependency's own declared range still permits a vulnerable release:
  ...
  - `@eslint/eslintrc`'s `js-yaml` dependency is bumped to `^4.3.1` to patch [GHSA-5p4m-2wfm-xmqj](https://github.com/advisories/GHSA-5p4m-2wfm-xmqj) (quadratic CPU consumption in `!!omap` resolution; the CVE-2026-59870 fix was not backported to 3.x/4.x). Needed because `eslint` → `@eslint/eslintrc` declares `js-yaml: ^4.1.1`, a range that still allows the unpatched `4.0.0`–`4.3.0`. Scoped to `@eslint/eslintrc` rather than declared globally so the top-level `js-yaml@5.3.0` is left untouched.
  ...
  - `brace-expansion: ^5.0.9` patches [GHSA-rgw5-rvv9-x895](https://github.com/advisories/GHSA-rgw5-rvv9-x895) (DoS via unbounded intermediate arrays, bypassing the [CVE-2026-14257](https://github.com/advisories/GHSA-mh99-v99m-4gvg) mitigation). Needed because `archiver` → `readdir-glob` → `minimatch` would resolve to a version below `5.0.9` without this override; the override was previously pinned at `^5.0.8`, which patched CVE-2026-14257 but was itself within the range vulnerable to GHSA-rgw5-rvv9-x895 (`4.0.0`–`5.0.8`).
  - `fast-uri: ^3.1.7` patches [CVE-2026-16221](https://github.com/advisories/GHSA-v2hh-gcrm-f6hx) and four more HIGH advisories fixed in 3.1.6 — [GHSA-5jgf-p345-68v8](https://github.com/advisories/GHSA-5jgf-p345-68v8) and [GHSA-jqff-g426-hqxp](https://github.com/advisories/GHSA-jqff-g426-hqxp) (host confusion via skipped IDN/percent-encoded scheme normalization), [GHSA-f65p-4m7j-42xc](https://github.com/advisories/GHSA-f65p-4m7j-42xc) and [GHSA-fph4-wmhf-6fwf](https://github.com/advisories/GHSA-fph4-wmhf-6fwf) (SSRF via malformed IPv6/repeated hostname percent-decoding normalization). Needed because `ajv@8.20.0`'s own declared range (`^3.0.1`, then `^3.1.5`) kept allowing an unpatched release. `ajv` is a shared transitive dependency of both `secretlint` (dev-only) and the production `@hookform/resolvers` (as a `peerOptional` dependency), so despite earlier notes here, `fast-uri` **is** reachable in production — confirmed by `npm audit --omit=dev` flagging it directly.
